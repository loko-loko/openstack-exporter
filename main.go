...
func main() {
	var (
		...
		addTenantName   = kingpin.Flag("tenant-name", "Add tenant name on volumes and servers metrics").Default().Bool()
	...
	for service, disabled := range services {
		if !*disabled {
			_, err := exporters.EnableExporter(service, *prefix, *cloud, *disabledMetrics, *endpointType, *addTenantName)
			if err != nil {
